...
<#-- page size -->
<#-- At first, I just changed the value of a parameter from 10 to 30 to make the pageSize
<#-- but decided that since the functions name is get, probably it will not set anything to 30, thus I just replaced to pageSize = 30 -->
<#assign pageSize = 30 />
<#-- As I understand, the pageSizeUser is the page size set by a user, but according to the task I need to make it constant. -->
<#--<#if user.registered>
...
<#switch sorting>
    ...
    <#break>
    <#-- order by post time descending, because the value of later is always more -->
    <#case "postTime">
        <#assign orderClause = "ORDER BY post_time DESC " />
    <#break>
    <#-- same as kudos, but ascending -->
    <#case "kudosAsc">
        <#assign orderClause = "ORDER BY kudos.sum(weight) ASC " />
    <#break>
    <#-- not sure what conversation.solved means, but I thought if it is unanswered the status should be 0 or low. -->
    <#case "queUnans">
        <#assign orderClause = "ORDER BY conversation.solved ASC " />
        <#-- SELECT * WHERE conversation.solved = 0 -->
    <#break>
    <#-- same logic here -->
    <#case "queNoSol">
        <#assign orderClause = "ORDER BY moderation_status ASC " />
        <#-- SELECT * WHERE moderation_status = 0 -->
    <#break>
    <#default>
       ...
       <header>
           <div class="custom-message-list-header-left">
               <select id="community-activity-sorted-by">
                   ...
                   <option value="post time" <#if sorting == 'postTime'>selected</#if>>${text.format("theme-lib.community-activity.postTime")}</option>
                   <option value="kudos ascending" <#if sorting == 'kudosAsc'>selected</#if>>${text.format("theme-lib.community-activity.kudosAsc")}</option>
                   <option value="questions unanswered" <#if sorting == 'queUnans'>selected</#if>>${text.format("theme-lib.community-activity.queUnans")}</option>
                   <option value="questions without soluion" <#if sorting == 'queNoSol'>selected</#if>>${text.format("theme-lib.community-activity.queNoSol")}</option>
                </select>
...
<#macro renderMessage msg showBoard=true isOdd=false isFirst=false isLast=false isFloated=false>
   ...
   <#-- I need to change the value here to change the name of the class. I choose to name it just thread-solved-->
   <#if msg.conversation.solved>
       <#assign solved = "thread-solved" />
